...
import pandas
import random
class EnglishForFrench:
    
    
    def __init__(self):
        try:
            self.data = pandas.read_csv("data/english_word_to_learn.csv")
        ...
        self.canvas2.create_text(180,60,text="English word:",font=("Ariel",20,"italic"))
        ...
        self.canvas2.create_text(180,300,text="French definition:",font=("Ariel",20,"italic"))
    ...
    def add_new_word(self):
        if self.new_word.get() == "" or self.new_word_definition.get() == "":
            ...
            self.data_list1.append({"french":self.new_word.get(),"english":self.new_word_definition.get()})
            self.new_word.delete(0,END)
            self.new_word_definition.delete(0,END)
            self.data = pandas.DataFrame(self.data_list1)
            self.data.to_csv("data/english_word_to_learn.csv",index=False)
        else:
            pass
    def change_word(self):
        ...
        self.canvas.itemconfig(self.word,text= self.dictionary["english"],fill="black")
        self.after_time = self.window.after(3000,func=self.translate)
    ...
    def translate(self):
        ...
        self.canvas.itemconfig(self.word,text=self.dictionary["french"],fill="white")
